...
import { ClientOptions } from '@oursprivacy/platform-sdk';

const prompt = `Runs JavaScript code to interact with the Ours Privacy Platform API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    OURS_PRIVACY_API_KEY: requireValue(
      readEnv('OURS_PRIVACY_API_KEY') ?? client.apiKey,
      'set OURS_PRIVACY_API_KEY environment variable or provide apiKey client option',
    ),
    OURS_PRIVACY_PLATFORM_BASE_URL: readEnv('OURS_PRIVACY_PLATFORM_BASE_URL') ?? client.baseURL ?? undefined,
  };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'ours-privacy-platform',
      code,
      intent,
      client_opts: {},
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@oursprivacy/platform-sdk';
