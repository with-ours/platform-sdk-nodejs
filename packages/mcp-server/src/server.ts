...
export const newMcpServer = async ({
  ...
  new McpServer(
    {
      name: 'oursprivacy_platform_sdk_api',
      version: '1.12.0',
    },
    {
...
export async function initMcpServer(params: {
  ...
  let _client: OursPrivacyPlatform | undefined;
  let _clientError: Error | undefined;
  let _logLevel: 'debug' | 'info' | 'warn' | 'error' | 'off' | undefined;

  const getClient = (): OursPrivacyPlatform => {
    if (_clientError) throw _clientError;
    if (!_client) {
      try {
        _client = new OursPrivacyPlatform({
          logger,
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    let client: OursPrivacyPlatform;
    try {
      client = getClient();
    } catch (error) {
      return {
